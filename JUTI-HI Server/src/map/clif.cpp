...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP] || (sd && sd->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)	WBUFL(buf,4) = ((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP] || (bl->type == BL_PC && ((TBL_PC *)bl)->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
		...
		((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP])<<5)| // COUNT_PK - Show the PvP counter
...
void clif_parse_ViewPlayerEquip(int fd, map_session_data* sd)
{
	...
	{	
	//else if( (pc_readaccountreg(tsd,add_str("#PRIVATE_VIEWEQUIP")) > 0 && tsd->status.show_equip) || pc_has_permission(sd, PC_PERM_VIEW_EQUIPMENT) )
		clif_viewequip_ack(sd, tsd);
		//atcommand_battlestats_sub(fd, tsd);
	}
	else
			...
			if( cost != nullptr ){
				...
				*/
				...
				if( sd->sc.getSCE(SC_MEMBERLV10) )
					Bonus3 = 5;		
				if( sd->sc.getSCE(SC_MEMBERLV11) )
					Bonus3 = 6;		
				if( sd->sc.getSCE(SC_MEMBERLV12) )
					Bonus3 = 7;		
				if( sd->sc.getSCE(SC_MEMBERLV13) )
					Bonus3 = 7;		
				if( sd->sc.getSCE(SC_MEMBERLV14) )
					Bonus3 = 8;		
				if( sd->sc.getSCE(SC_MEMBERLV15) )
					Bonus3 = 9;		
				if( sd->sc.getSCE(SC_MEMBERLV16) )
					Bonus3 = 9;		
				if( sd->sc.getSCE(SC_MEMBERLV17) )
					Bonus3 = 10;		
				if( sd->sc.getSCE(SC_MEMBERLV18) )
					Bonus3 = 11;		
				if( sd->sc.getSCE(SC_MEMBERLV19) )
					Bonus3 = 12;		
				if( sd->sc.getSCE(SC_MEMBERLV20) )
					Bonus3 = 13;		
				if( sd->sc.getSCE(SC_MEMBERLV21) )
					Bonus3 = 14;		
				if( sd->sc.getSCE(SC_MEMBERLV22) )
					Bonus3 = 15;		
				if( sd->sc.getSCE(SC_MEMBERLV23) )
					Bonus3 = 16;		
				if( sd->sc.getSCE(SC_MEMBERLV24) )
					Bonus3 = 17;		
				if( sd->sc.getSCE(SC_MEMBERLV25) )
					Bonus3 = 17;		
				if( sd->sc.getSCE(SC_MEMBERLV26) )
					Bonus3 = 18;		
				if( sd->sc.getSCE(SC_MEMBERLV27) )
					Bonus3 = 18;		
				if( sd->sc.getSCE(SC_MEMBERLV28) )
					Bonus3 = 18;		
				if( sd->sc.getSCE(SC_MEMBERLV29) )
					Bonus3 = 19;		
				if( sd->sc.getSCE(SC_MEMBERLV30) )
					Bonus3 = 20;	
...
void clif_parse_refineui_refine( int fd, map_session_data* sd ){
	...
	int Finalrate = cost->chance;	
				...
				if( sd->sc.getSCE(SC_REFINEPLUS2) ){
					Bonus44 = sd->sc.getSCE(SC_REFINEPLUS2)->val1*100;		
				}
				
				if( sd->sc.getSCE(SC_REFINEPLUS) ){
					Bonus22 = sd->sc.getSCE(SC_REFINEPLUS)->val1*100;		
				}
				/*
				if( sd->sc.getSCE(SC_MEMBERLV1) )
					Bonus33 = 1;		
	...
	if( Finalrate >= ( rnd() % 10000 ) ){
		if (sd->sc.getSCE(SC_FAILSTACK))
			status_change_end(&sd->bl, SC_FAILSTACK);
		...
		if( battle_config.failstackevent) {
			if( sd->sc.getSCE(SC_FAILSTACK) ){
				sc_start(&sd->bl, &sd->bl, SC_FAILSTACK, 100, 1 + sd->sc.getSCE(SC_FAILSTACK)->val1 , 600000);
				count = sd->sc.getSCE(SC_FAILSTACK)->val1;
				sprintf(atcmd_output, msg_txt(sd,1526), count); // Fail Stack LV. %d
				clif_displaymessage(fd, atcmd_output);
			}else{
				sc_start(&sd->bl, &sd->bl, SC_FAILSTACK, 100, 1, 600000);
				count = sd->sc.getSCE(SC_FAILSTACK)->val1;
				sprintf(atcmd_output, msg_txt(sd,1526), count); // Fail Stack LV. %d
				clif_displaymessage(fd, atcmd_output);
			}
