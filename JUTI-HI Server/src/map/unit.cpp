	...
	if (bl->type == BL_PC) {
		...
		if (sd && sd->pvpcan_walkout_tick && !map_getcell(sd->bl.m, x, y, CELL_CHKPVP)) {
			if (DIFF_TICK(tick, sd->pvpcan_walkout_tick) < battle_config.cellpvp_walkout_delay) {
				...
				char message[150];
				if (e_tick > 99) {
					sprintf(message, msg_txt(sd, 1610), sd->status.name, (double)e_tick / 60);
					clif_displaymessage(sd->fd, message);
				} else {
					sprintf(message, msg_txt(sd, 1611), sd->status.name, e_tick + 1);
					clif_displaymessage(sd->fd, message);
				}
				return 0;
			}
...
int unit_skilluse_id(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv)
{
	...
	if (bl && src->type == BL_PC && bl->type == BL_PC && src->id != target_id) {
		map_session_data *dstsd = (TBL_PC*)bl;
