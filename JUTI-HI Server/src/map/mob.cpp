...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, mob_data* md, enum item_types type)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if (sd->sc.getSCE(SC_EQ_BUBBLE_GUM100) && (type == IT_WEAPON || type == IT_ARMOR) )
			...
			if( map_getmapflag(src->m, MF_DROPEVENT3) )
				drop_rate = drop_rate * battle_config.MF_DROPEVENT3 / 100;
