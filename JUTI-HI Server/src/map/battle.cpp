	...
	switch( attack_type ) {
		case BF_MAGIC:
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
				...
				if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					int ele_fix = cap_value(tsd->indexed_bonus.subele[rh_ele] + tsd->indexed_bonus.subele[ELE_ALL] + tsd->indexed_bonus.subele_script[rh_ele] + tsd->indexed_bonus.subele_script[ELE_ALL],0,battle_config.SUBELECAP);	/*extreme_custom_src*/	
				...
				race_fix = cap_value(tsd->indexed_bonus.subrace[sstatus->race] + tsd->indexed_bonus.subrace[RC_ALL],0,battle_config.SUBDEFCAP);	/*extreme_custom_src*/	
		...
		case BF_MISC:
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
				...
				int race_fix = cap_value(tsd->indexed_bonus.subrace[sstatus->race] + tsd->indexed_bonus.subrace[RC_ALL],0,battle_config.SUBDEFCAP);	/*extreme_custom_src*/	
				for (const auto &it : tsd->subrace3) {
					if (it.race != RC_ALL && it.race != sstatus->race)
						continue;
					if (!(((it.flag)&flag)&BF_WEAPONMASK &&
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Non pvp/gvg, check party/guild settings.
		...
		if( flag&BCT_GUILD || state&BCT_ENEMY )
		{
			int s_guild = status_get_guild_id(s_bl);
			int t_guild = status_get_guild_id(t_bl);
			if (s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) && !(battle_config.cellpvp_guild_enable && map_getcell(t_bl->m, t_bl->x, t_bl->y, CELL_CHKPVP)))		// Cell PvP
 				state |= BCT_GUILD;
			else
				state |= BCT_ENEMY;
		}
...
// rAthena
	...
	{ "SUBELECAP",             				&battle_config.SUBELECAP,  		        	   100,    	0,      100,         	},
	{ "SUBDEFCAP",             				&battle_config.SUBDEFCAP,  		        	   100,    	0,      100,         	},
	{ "MF_DROPEVENT1",             			&battle_config.MF_DROPEVENT1,  		        	 0,    	0,      1000,         	},
	{ "MF_DROPEVENT2",             			&battle_config.MF_DROPEVENT2,  		        	 0,    	0,      1000,         	},
	{ "MF_DROPEVENT3",             			&battle_config.MF_DROPEVENT3,  		        	 0,    	0,      1000,         	},
	{ "failstackevent",              		&battle_config.failstackevent,           		 0,     0,      1, },
