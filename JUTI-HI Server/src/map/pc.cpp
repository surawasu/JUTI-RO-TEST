...
void pc_makesavestatus(map_session_data *sd) {
	...
	if( pc_isdead( sd ) ){
		...
		if( mapdata->flag[MF_NOSAVE] || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP)){
...
bool pc_isUseitem(map_session_data *sd,int n)
{
	...
	if( itemdb_group.item_exists( IG_MF_NORETURN, nameid ) || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP)){
...
bool pc_memo(map_session_data* sd, int pos)
{
	...
	// check mapflags
	if( sd->bl.m >= 0 && (map_getmapflag(sd->bl.m, MF_NOMEMO) || map_getmapflag(sd->bl.m, MF_NOWARPTO)) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE) || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP)) {	// Cell PvP
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
	...
	if(battle_config.bone_drop==2
		|| (battle_config.bone_drop==1 && mapdata->flag[MF_PVP]) || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKPVP) && sd->state.pvp) {
